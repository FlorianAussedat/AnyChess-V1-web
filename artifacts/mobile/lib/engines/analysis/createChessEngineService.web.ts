 ...
 *
 * Import path MUST be `../stockfish/transport` (no `.ts`) so Metro picks
 * `transport.web.ts` and not the native stub.
 */
...
export function createChessEngineService(
  ...
  return new ChessEngineService({
    ...options,
    createTransport: options.createTransport ?? createUciTransport,
  });
}
